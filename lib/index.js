...
const LOG_FILE = process.env.OC_LOG_FILE || join(HOME, '.dsh', 'openchronicle.log');
function logLine(msg) {
  try {
    fsSync.appendFileSync(LOG_FILE, `[${new Date().toISOString()}] ${msg}\n`);
  } catch (e) { /* 日志失败不阻塞 */ }
}
...
export function apply(ctx) {
  // R014 自查门：依赖完整性检查
  try { runSelfCheck('openchronicle', { requiredPeers: ['@deepseek-ai/dsh-tools'], requiredSymbols: ['join', 'homedir'] }); } catch (e) { /* 自查失败不阻塞 */ }
  startAdaptGuard(ctx, 'openchronicle');
  const toolsSvc = ctx.get('tools', false);
  if (!toolsSvc) { logLine('[openchronicle] tools 服务不可用'); return; }
  ...
  if (CAPTURE_INTERVAL_MIN > 0) {
    timer = setInterval(async () => {
      ...
      if (!helper) { logLine('[openchronicle] helper 未找到，跳过定时捕获'); return; }
