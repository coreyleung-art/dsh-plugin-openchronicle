...
//
// 工具：
//   oc_capture      — 手动触发一次捕获（当前前台 app）
//   oc_current_context — agent 查「用户此刻在做什么」（最新捕获）
//   oc_search       — 检索历史捕获记忆
//   oc_memories     — 列出记忆文件
//
// 设计：约定式 cordis 插件（export name/inject/apply），spawn 外部二进制，
//       不绑定端口。捕获节流 + 记忆软/硬上限防 token 浪费。
...
const MAX_MEMORY_FILES = 500; // 记忆文件上限（防膨胀）
const MAX_MEMORY_MB = 50;     // 记忆目录上限（防磁盘膨胀）
...
async function writeCapture(dateStr, summary, rawJson) {
  ...
  const entry = `\n## ${ts}\n${summary}\n`;
  content += entry;
  await fs.writeFile(file, content.slice(-200000)); // 每文件上限 200KB
  // 可选存原始 JSON（调试）
...
export function apply(ctx) {
  ...
  if (!tools) { console.log('[openchronicle] tools 服务不可用'); return; }
  ...
  // 工具 2: oc_current_context — agent 查用户此刻在做什么
  defineTool({
  ...
  // 工具 5: oc_snapshot — 把当前感知记忆沉淀为知识库文档（按需，防 token 浪费）
  defineTool({
    name: 'oc_snapshot',
    ...
    handler: async (args) => {
      ...
      // 可选：追加到 knowledge 库（调用 knowledge 服务）
  ...
  // 工具 6: oc_timeline — 感知记忆时间线（AriadneMem 式状态迁移去重）
  // 同一应用的连续捕获若内容相同（状态未变），合并为一条并标注持续时长，
  // 只有状态迁移（应用切换/内容变化）才新增条目——防重复、省 token。
  defineTool({
    name: 'oc_timeline',
  ...
  // 工具 7: oc_share — 感知记忆跨设备共享（黑板 data/perception/ 通道）
  // mac 端捕获的感知记忆可同步到黑板，i9/MBP 可读（跨设备记忆统一）
  defineTool({
    name: 'oc_share',
